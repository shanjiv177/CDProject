...
\begin{tikzpicture}[
    ...
    \node[state, fill=red!10] (q6) at (7,-6) {$q_6$};
    ...
    \draw[->, thick] (q3) edge[loop below] node {any except * or /} (q3);
    \draw[->, thick] (q3) -- node[above] {*} (q4);
    \draw[->, thick] (q3) -- node[left] {/} (q6);
    \draw[->, thick] (q6) -- node[below] {*} (q3);
    \draw[->, thick] (q6) edge[loop below] node {any except *} (q6);
    ...
    \node[anchor=west] at (7,0) {
        \begin{tabular}{l}
            ...
            - Nested comments trigger warning
        \end{tabular}
    };
\end{tikzpicture}
...
\subsection{Test Case 6: Advanced String and Character Literals}
...
\subsection{Test Case 7: Custom Functions with Parameters and Calls}
